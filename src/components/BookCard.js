...
export const BookCard = ({ setKeyword, ...book }) => {
  ...
  const addImage = (e) => {
    e.preventDefault()
    fetch(`https://philo-biblist.herokuapp.com/books/${book.bookID}`, {
      method: "PUT",
      mode: 'cors',
      headers: {
  ...
  return (
    <CardContainer>
      <ImageWrapper>
        {!book.img_url && <Image src={require('../blank-cover.jpg')} />}
        {book.img_url && <Image src={book.img_url} />}
      ...
      <InfoWrapper>
        ...
        <RatingsInfoWrapper>
          ...
          {showTextField && <form onSubmit={(e) => addImage(e)}>
            <input type="url" required placeholder="Add image url" onChange={(e) => setImgUrl(e.target.value)}></input>
